...
The [official area stop/search contract](https://data.police.uk/docs/method/stops-street/) was rechecked on 2026-09-16 and its first example record frozen in tests. Locations are approximate. Timestamps are preserved without imposing an additional returned-month equality rule. The service holds the shared admission lease through publication, writes header-only empty results, and uses the common cancellation and sanitized error behavior. Full operation budgets/input limits remain Stage 7; host binding integration remains Stage 8.
