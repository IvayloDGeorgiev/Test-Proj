...
The existing .NET 10 controller application now provides **POST /api/ingestion/forces** and **POST /api/ingestion/crimes**, with validated configuration, reusable Police API transport, safe CSV storage, and an xUnit suite. Stop/search ingestion remains planned. The WeatherForecast template has been removed.
...
`LocationMonth.Create` validates nullable finite coordinates and exact ASCII calendar months before query formatting; query numbers use invariant culture. Crime retrieval now uses these reusable values; stop/search remains planned. `PoliceApiException` contains safe codes/statuses only; centralized middleware translates ingestion failures into sanitized ProblemDetails in both Development and Production. Transport logs safe dataset/trace/attempt/status-class/count/elapsed/result fields and suppresses the factory's URL logging. Tests use scripted handlers and a manual clock, never live Police API calls or real retry sleeps.
