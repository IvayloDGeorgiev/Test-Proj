...
Stage 7 wraps ingestion paths in IngestionLimitsMiddleware inside the error middleware. A bounded in-memory pre-read (limit + 1 maximum) rejects oversized known/unknown-length requests before binding and sets the server request-size feature when writable. An async-local OperationDeadline isolates each request's monotonic remaining budget and original caller token; the typed transport consults it before retry delay admission. A linked TimeProvider timer propagates the deadline via RequestAborted through the existing service/export boundaries. Timeout becomes operation_timeout/504 only when the caller has not cancelled. The original body/token/ambient state are restored in finally. No service/export retry is added.

IngestionResultFilter restores the original caller token after the service returns its published result, before result serialization, and logs safe publication metadata. The timer never undoes committed output. Cancellation-aware stream writes/flush and the pre-publication check stop uncommitted work; synchronous OS durability/rename/cleanup calls remain cooperative OS boundaries and are not detached into background work. Framework 400/415 results and validation exceptions use fixed sanitized fields/messages; all operational errors retain code/traceId and application/problem+json. Controllers declare these responses in OpenAPI. Stage 8 verifies actual host binding, filter ordering and generated OpenAPI end to end.

## Final host verification

Stage 8 adds Microsoft.AspNetCore.Mvc.Testing 10.0.12 to the existing net10.0 xUnit project and a public partial Program entry point. WebApplicationFactory executes the production registration/pipeline with a temporary content root, synthetic configuration, scripted HttpMessageHandler, manual TimeProvider and temporary real filesystem output. Fault tests substitute only the OS boundary; controllers, mapping, transport policy, middleware, filters, admission and exporter remain real.

Generated OpenAPI originally represented the count as integer-or-string due to ASP.NET JSON number handling, and the nullable controller parameters advertised optional/null bodies despite required validation. RecordCount now explicitly uses strict numeric metadata; the two body parameters are nonnullable while service/request field validation remains defensive and nullable. Generated-document tests assert required bodies, integer counts, exact routes/status sets, success/validation/problem schemas and problem media types. Runtime binding regressions cover missing/null/malformed bodies and unchanged sanitized 400 behavior.

Host verification covers exact CSV bytes, Unicode/formula escaping, nullable mapping, empty replacement, the full HTTP error table, safe logs in both environments, all nine retrieval contention pairs, contention during writes, actual cancellation propagation/cleanup, attempt versus operation timeout, input time consuming retry budget, and complete response serialization when the deadline expires after atomic publication. No live upstream or actual Desktop/configuration is used. TestServer does not replace deployment TLS/web-server verification. OS ownership, multi-process exclusion and local filesystem semantics remain the explicit supported boundaries in SECURITY.md.
